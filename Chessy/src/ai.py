...
class Heuristics:
    ...
    def evaluate(board):
        ...
        pawns = Heuristics.get_piece_position_score(board, piece.Pawn.PIECE_TYPE, Heuristics.PAWN_TABLE)
        knights = Heuristics.get_piece_position_score(board, piece.Knight.PIECE_TYPE, Heuristics.KNIGHT_TABLE)
        bishops = Heuristics.get_piece_position_score(board, piece.Bishop.PIECE_TYPE, Heuristics.BISHOP_TABLE)
        rooks = Heuristics.get_piece_position_score(board, piece.Rook.PIECE_TYPE, Heuristics.ROOK_TABLE)
        queens = Heuristics.get_piece_position_score(board, piece.Queen.PIECE_TYPE, Heuristics.QUEEN_TABLE)
...
class AI:
    ...
    def get_ai_move(chessboard, invalid_moves):
        ...
        if (copy.is_check(piece.Piece.BLACK)):
    ...
    def minimax(board, depth, maximizing):
        ...
        if (maximizing):
            best_score = -AI.INFINITE
            for move in board.get_possible_moves(piece.Piece.WHITE):
            ...
            best_score = AI.INFINITE
            for move in board.get_possible_moves(piece.Piece.BLACK):
                copy = board.Board.clone(board)
                copy.perform_move(move)
    ...
    def alphabeta(chessboard, depth, a, b, maximizing):
        ...
        if (maximizing):
            best_score = -AI.INFINITE
            for move in chessboard.get_possible_moves(piece.Piece.WHITE):
                copy = board.Board.clone(chessboard)
                copy.perform_move(move)
